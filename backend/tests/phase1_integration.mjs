...
if (!superEmail || !superPassword) {
  throw new Error('Set PB_SUPER_EMAIL and PB_SUPER_PASSWORD before running this integration test.')
}
...
async function createUser(label, role = 'proofreader') {
  ...
  if (role !== 'proofreader') {
    await request(`/api/collections/users/records/${record.id}`, {
      method: 'PATCH',
      token: superToken,
      body: { role }
    })
...
try {
  const admin = await createUser('phase1-admin', 'admin')
