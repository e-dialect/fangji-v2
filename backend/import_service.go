...
func (s *importService) processCSVInspection(work importWork) {
	...
	pdfPageLimit := s.primaryPDFPageCount(job.GetString("project"))
	for {
		...
		if _, validationErr := buildCSVPageWithLimit(headers, values, pdfIndex, line, pdfPageLimit); validationErr != nil {
			inspection.InvalidRows++
			s.addJobError(
				work.id,
				validationErr.line,
...
func (s *importService) primaryPDFPageCount(projectID string) int {
	...
	return records[0].GetInt("page_count")
}
