...
- 项目大厅按项目汇总一校、二校、我的进行中和完成进度，并直接标明下一步是继续、一校还是二校。
...
2. 进入“项目大厅”。
3. 查看各项目的一校待领、二校待领、我的进行中和完成进度。
4. 点击“领取一校”“领取二校”或“继续校对”。按钮下方会预告即将打开的条号和轮次。
...
编辑页左侧显示 PDF 原文，右侧按纵向字段卡显示 CSV 内容和 IPA/BUC 字符板。字段卡始终保留导入原文，修改后的字段会明确标记，也可以逐项恢复原文。

操作流程：

1. 根据 `PDF页码` 查看对应 PDF 页；界面允许在当前页和下一页之间切换，并支持缩放、适宽、旋转和全屏。
2. 在右侧字段卡中逐项校对；顶部会显示当前任务位置、校对轮次、草稿状态和修改字段数。
3. 展开 IPA/BUC 字符板中需要的分组，在当前光标处插入特殊字符，也可以替换已选中的文本。
...
2. 仲裁台默认只显示不一致字段；逐项采用原文、一校、二校或手工编辑，也可以批量采用一个来源。
