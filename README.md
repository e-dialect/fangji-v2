...
创建者自动成为项目唯一所有者。所有者和项目管理员可在项目设置中添加成员、配置启用键盘和默认键盘，并在“公开加入”“指定成员”“口令加入”之间切换；切换方式或修改口令不会移除已有成员。删除项目或转移所有权会立即释放原所有者占用的创建额度。新旧项目默认启用“莆仙方言键盘”。
...
同一条目的两份独立结果必须来自不同校对员。如果剩余条目都包含自己的既有提交，界面只提示暂无可处理任务，不暴露后台轮次。
...
2. 仲裁台左侧显示当前条目关联的 PDF，右侧默认只显示不一致字段；可逐项采用原文、任一校对结果或手工编辑，也可以批量采用一个来源。
...
└── backend/
    ...
    └── pb_migrations/              # Schema、权限、导入作业、键盘和双人二校迁移
```
